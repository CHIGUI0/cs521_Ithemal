...
def save_dataset(dataset, arch):
    ...
    try:
        ...
        #         # Write block_id (Python 2 str from CSV is already bytes)
        ...
        #         # Write code_intel (was unicode, encode to utf-8 bytes)
        #         intel_bytes = code_intel.encode('utf-8')
        #         f.write(struct.pack('i', len(intel_bytes)))
        #         f.write(intel_bytes)
                
        #         # Write code_xml (was unicode, encode to utf-8 bytes)
...
if __name__ == "__main__":
    # Protect the entry point when using multiprocessing, good practice though mainly for Windows.
    # In Python 2, this is typically not needed for Pool on Linux, but harmless.
    # If using 'spawn' or 'forkserver' start methods (more Py3), it's essential.
    # Python 2 on Linux defaults to 'fork', where freeze_support() is less critical.
